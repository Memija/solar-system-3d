...
export class Modal {
    ...
    private getExtraInfo(data: ModalData): string {
        ...
        if ('radius' in data) {
            const displayRadius = data.displayRadius !== undefined ? data.displayRadius : data.radius;
            ...
            if ('semiMajorAxis' in data) {
                ...
                info += `<p style="display: flex; align-items: center;">${createInfoButton("Distance", "The average distance from the Sun, measured in Astronomical Units (AU). One AU is the average distance from Earth to the Sun.")}<strong>Distance:</strong>&nbsp;${body.distance} AU</p>
